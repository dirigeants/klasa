...
const type_1 = require("@klasa/type");
...
class default_1 extends klasa_1.Command {
    ...
    async run(message, [code]) {
        ...
        const footer = discord_md_tags_1.codeblock('ts') `${type}`;
        const output = message.language.get(success ? 'COMMAND_EVAL_OUTPUT' : 'COMMAND_EVAL_ERROR', time, discord_md_tags_1.codeblock('js') `${result}`, footer);
        if ('silent' in message.flagArgs)
            return [];
    ...
    async eval(message, code) {
        ...
        try {
            ...
            if (!syncTime)
                syncTime = stopwatch.toString();
            if (!type)
                type = new type_1.Type(error);
            if (thenable && !asyncTime)
                asyncTime = stopwatch.toString();
            if (error && error.stack)
                this.client.emit('error', error.stack);
            result = error;
            success = false;
        }
        stopwatch.stop();
