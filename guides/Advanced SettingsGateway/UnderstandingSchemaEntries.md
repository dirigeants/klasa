...
```javascript
const filter = (client, command, entry, guild) => {
...
In this case, `client` is the {@link KlasaClient} instance, `command` the resolved command (the output from the command's SchemaType), `entry` is a {@link SchemaEntry} instance, and guild is a {@link Guild} instance, which may be null.
