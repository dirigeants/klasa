...
module.exports = class extends Command {
	...
	show(message, [key]) {
		...
		return message.sendLocale('COMMAND_CONF_GET', { localeArgs: [path.piece.path, message.guild.configs.resolveString(message, path.piece)] });
	}
	...
	async set(message, [key, ...valueToSet]) {
		...
		if (!updated.length) return message.sendLocale('COMMAND_CONF_NOCHANGE', { localArgs: [key] });
		return message.sendLocale('COMMAND_CONF_UPDATED', { localArgs: [key, message.guild.configs.resolveString(message, updated[0].piece)] });
	}
	...
	async reset(message, [key]) {
		...
		if (errors.length) return message.sendMessage(errors[0]);
		if (!updated.length) return message.sendLocale('COMMAND_CONF_NOCHANGE', { localArgs: [key] });
		return message.sendLocale('COMMAND_CONF_RESET', { localArgs: [key, message.guild.configs.resolveString(message, updated[0].piece)] });
	}
