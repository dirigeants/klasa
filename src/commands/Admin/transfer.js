...
module.exports = class extends Command {
	...
	async run(message, [piece]) {
		...
		try {
			...
			return message.sendLocale('COMMAND_TRANSFER_SUCCESS', { localeArgs: [piece.type, piece.name] });
		} catch (err) {
			this.client.emit('error', err.stack);
			return message.sendLocale('COMMAND_TRANSFER_FAILED', { localeArgs: [piece.type, piece.name] });
		}
	}
