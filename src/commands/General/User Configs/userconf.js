...
module.exports = class extends Command {
	...
	show(message, [key]) {
		...
		if (!path) return message.sendLocale('COMMAND_CONF_GET_NOEXT', { localArgs: [key] });
		...
		return message.sendLocale('COMMAND_CONF_GET', { localeArgs: [path.piece.path, message.author.configs.resolveString(message, path.piece)] });
	}
	...
	async set(message, [key, ...valueToSet]) {
		...
		if (!updated.length) return message.sendLocale('COMMAND_CONF_NOCHANGE', { localeArgs: [key] });
		return message.sendLocale('COMMAND_CONF_UPDATED', { localeArgs: [key, message.author.configs.resolveString(message, updated[0].piece)] });
	}
	...
	async reset(message, [key]) {
		...
		if (errors.length) return message.sendMessage(errors[0]);
		if (!updated.length) return message.sendLocale('COMMAND_CONF_NOCHANGE', { localeArgs: [key] });
		return message.sendLocale('COMMAND_CONF_RESET', { localeArgs: [key, message.author.configs.resolveString(message, updated[0].piece)] });
	}
