...
module.exports = class extends Command {
	...
	async run(msg, [piece]) {
		if ((piece.type === 'event' && piece.name === 'Message') || (piece.type === 'monitor' && piece.name === 'commandHandler')) {
			return msg.sendMessage(await msg.fetchLanguageCode('COMMAND_DISABLE_WARN'));
		}
		piece.disable();
		return msg.sendCode('diff', await msg.fetchLanguageCode('COMMAND_DISABLE', piece.type, piece.name));
	}
