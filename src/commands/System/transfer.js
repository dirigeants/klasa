...
module.exports = class extends Command {
	...
	async run(msg, [piece]) {
		...
		const language = await msg.fetchLanguage();
		await fs.access(fileLocation).catch(() => { throw language.get('COMMAND_TRANSFER_ERROR'); });
		return fs.copy(fileLocation, resolve(this.client.clientBaseDir, `${piece.type}s`, file))
			.then(() => {
				...
				return msg.sendMessage(language.get('COMMAND_TRANSFER_SUCCESS', piece.type, piece.name));
			})
			.catch((err) => {
				this.client.emit('error', err.stack);
				return msg.sendMessage(language.get('COMMAND_TRANSFER_FAILED', piece.type, piece.name));
			});
	}
