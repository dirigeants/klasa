...
class CommandMessage {
	...
	async validateArgs() {
		...
		if (this._currentUsage.type === 'optional' && (this.args[this.params.length] === undefined || this.args[this.params.length] === '')) {
			if (this.cmd.usage.parsedUsage.slice(this.params.length).some(usage => usage.type === 'required')) {
				...
				throw this.client.methods.util.newError(await this.msg.fetchLanguageCode('COMMANDMESSAGE_MISSING'), 1);
			} else {
				return this.params;
			}
			...
			throw this.client.methods.util.newError(this._currentUsage.possibles.length === 1 ?
				await this.msg.fetchLanguageCode('COMMANDMESSAGE_MISSING_REQUIRED', this._currentUsage.possibles[0].name) :
				await this.msg.fetchLanguageCode('COMMANDMESSAGE_MISSING_OPTIONALS', this._currentUsage.possibles.map(poss => poss.name).join(', ')), 1);
	...
	async multiPossibles(possible, validated) {
		if (validated) {
			...
			this.args.splice(this.params.length, 1, null);
			throw this.client.methods.util.newError(await this.msg.fetchLanguageCode('COMMANDMESSAGE_NOMATCH', this._currentUsage.possibles.map(poss => poss.name).join(', ')), 1);
	...
	static getArgs(cmdMsg) {
		...
		const args = cmdMsg.msg.content.slice(cmdMsg.prefixLength).trim().split(/\s+/g).slice(1).join(' ').trim().split(cmdMsg.cmd.usageDelim !== '' ? cmdMsg.cmd.usageDelim : undefined);
	...
	static getQuotedStringArgs(cmdMsg) {
		const content = cmdMsg.msg.content.slice(cmdMsg.prefixLength).trim().split(/\s+/g).slice(1).join(' ').trim();
