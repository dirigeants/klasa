...
module.exports = class extends Provider {
	 ...
	 */
	async init() {
		await fs.ensureDir(baseDirectory).catch(err => this.client.emit('error', err));
	}
	 ...
	 */
	hasTable(table) {
		return fs.pathExists(resolve(baseDirectory, table));
	}
	 ...
	 */
	createTable(table) {
		return fs.mkdir(resolve(baseDirectory, table));
	}
	...
	deleteTable(table) {
		return this.hasTable(table)
			.then(exists => exists ? fs.emptyDir(resolve(baseDirectory, table)).then(() => fs.remove(resolve(baseDirectory, table))) : null);
	}
	 ...
	 */
	async getKeys(table) {
		const dir = resolve(baseDirectory, table);
	 ...
	 */
	get(table, id) {
		return fs.readJSON(resolve(baseDirectory, table, `${id}.json`)).catch(() => null);
	}
	 ...
	 */
	has(table, id) {
		return fs.pathExists(resolve(baseDirectory, table, `${id}.json`));
	}
	...
	create(table, document, data = {}) {
		return fs.outputJSONAtomic(resolve(baseDirectory, table, `${document}.json`), { id: document, ...data });
	}
	...
	async update(table, document, data) {
		...
		return fs.outputJSONAtomic(resolve(baseDirectory, table, `${document}.json`), util.mergeObjects(existent || { id: document }, this.parseUpdateInput(data)));
	}
	...
	replace(table, document, data) {
		return fs.outputJSONAtomic(resolve(baseDirectory, table, `${document}.json`), { id: document, ...this.parseUpdateInput(data) });
	}
	...
	delete(table, document) {
		return fs.unlink(resolve(baseDirectory, table, `${document}.json`));
	}
