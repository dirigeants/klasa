...
```javascript
this.client.settings.guilds.schema.addKey(key, options, force?);
```
...
```javascript
this.client.settings.guilds.schema.addKey('modlogs', { type: 'TextChannel' });
```
...
```javascript
this.client.settings.guilds.schema.addKey('users', { type: 'User', array: true });
```
...
```javascript
this.client.settings.guilds.updateOne(msg.guild, 'modlogs', '267727088465739778');
```
...
```javascript
this.client.settings.guilds.updateArray(msg.guild, 'add', 'users', '146048938242211840');
```

That will add the user `'146048938242211840'` to the `users` array. To remove it:

```javascript
this.client.settings.guilds.updateArray(msg.guild, 'remove', 'users', '146048938242211840');
```
...
```javascript
this.client.settings.guilds.schema.removeKey('users');
```
...
```javascript
this.client.settings.guilds.schema.addFolder(name, object, force);
```
...
```javascript
const { schema } = this.client.settings.guilds;
async function() {
